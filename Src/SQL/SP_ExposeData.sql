...
/****** Object:  StoredProcedure [dbo].[SP_ExposeData]    Script Date: 04/19/2012 10:35:03 ******/
SET ANSI_NULLS ON
GO
...
IF OBJECT_ID(N'[TBL_RootInspections]', N'U') IS NOT NULL
	DROP TABLE TBL_RootInspections

CREATE TABLE TBL_RootInspections
	(
...
IF OBJECT_ID(N'[TBL_GreaseInspections]', N'U') IS NOT NULL
	DROP TABLE TBL_GreaseInspections

CREATE TABLE TBL_GreaseInspections
	(
		COMPKEY		int,			INSPKEY		int,		OBRATING		float,		
		[FROM]		float,			[TO]		float,	
		STARTDTTM	datetime,		COMPDTTM	datetime,	INSTDATE	datetime,
...
INSERT	INTO  TBL_GreaseInspections 
...
--sanitary inspections for grease accumulation
INSERT	INTO  TBL_RootInspections 
SELECT	C2.COMPKEY AS COMPKEY,
		C2.INSPKEY AS INSPKEY, 
		OBRATING AS OBRATING, 
		C3.DISTFROM AS [FROM], 

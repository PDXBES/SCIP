CREATE VIEW [dbo].[VW_CLEANING_ACTIVITIES_COUNT_BY_DRIVER_TYPE_FISCAL_YEAR]
  ...
  FROM (VW_ALL_ACTIVITIES A INNER JOIN ACTIVITY_TYPES B ON (A.activity_type_id = B.activity_type_id)) INNER JOIN ALTERNATIVES C ON (A.alternative_id = C.alternative_id)

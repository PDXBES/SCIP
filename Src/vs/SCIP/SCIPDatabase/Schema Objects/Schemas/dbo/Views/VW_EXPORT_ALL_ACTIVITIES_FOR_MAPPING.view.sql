CREATE VIEW [dbo].[VW_EXPORT_ALL_ACTIVITIES_FOR_MAPPING]
  ...
  FROM ((VW_ALL_ACTIVITIES A INNER JOIN DRIVERS B ON (A.DRIVER_ID = B.driver_id)) INNER JOIN ACTIVITY_TYPES C ON (A.ACTIVITY_TYPE_ID = C.activity_type_id)) INNER JOIN BASINS E ON (A.basin = E.name)

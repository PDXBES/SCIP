...
FROM (((VW_NEXT_ACTIVITIES_BY_COMPKEY A INNER JOIN DRIVERS B ON (A.DRIVER_ID = B.driver_id)) INNER JOIN ACTIVITY_TYPES C ON (A.ACTIVITY_TYPE_ID = C.activity_type_id)) INNER JOIN ACTIVITIES_FOR_PROCESSING D ON (A.COMPKEY = D.COMPKEY AND A.DRIVER_ID = D.driver_id AND A.NEXTACTIVI = D.activity_date)) INNER JOIN BASINS E ON (D.basin = E.name)

CREATE PROCEDURE [dbo].[SP_EXPORT_ACTIVITIES_FOR_MAPPING]
  @alternative_id INT
AS
BEGIN
  SET NOCOUNT ON
  ...
  SELECT
     X.compkey
    ,DATEPART(year, X.activity_date) AS NEXTACTIVI
    ,X.driver_type + ' ' + X.activity_type AS ACTIVITY
    ,X.activity_type
    ,X.cost
    ,X.basin_id
  FROM
    ...
    ORDER BY X.activity_type, X.basin_id, X.compkey
  
  EXEC SP_STATUS_MESSAGE 'End SP_EXPORT_ACTIVITIES_FOR_MAPPING'
END
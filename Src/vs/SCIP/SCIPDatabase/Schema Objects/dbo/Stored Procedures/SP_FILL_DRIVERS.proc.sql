...
BEGIN
  ...
  DECLARE @DRIVERS_ALL_COMPKEYS_FOR_PROCESSING TABLE (COMPKEY int)
  DECLARE @DRIVERS_LARGE_COMPKEYS_FOR_PROCESSING TABLE (COMPKEY int)
  DECLARE @DRIVERS_LARGE_ROOT_LARGE_PIPE_COMPKEYS_FOR_PROCESSING TABLE (COMPKEY int)
  DECLARE @DRIVERS_LARGE_ROOT_XOR_LARGE_PIPE_COMPKEYS_FOR_PROCESSING TABLE (COMPKEY int)
  DECLARE @DRIVERS_LARGE_TRACTIVE_COMPKEYS_FOR_PROCESSING TABLE (COMPKEY int)
  DECLARE @DRIVERS_SMALL_COMPKEYS_FOR_PROCESSING TABLE (COMPKEY int)
  DECLARE @DRIVERS_SMALL_ROOT_COMPKEYS_FOR_PROCESSING TABLE (COMPKEY int)
  DECLARE @DRIVERS_SMALL_TRACTIVE_COMBO_COMPKEYS_FOR_PROCESSING TABLE (COMPKEY int)
  DECLARE @DRIVERS_SMALL_TRACTIVE_COMPKEYS_FOR_PROCESSING TABLE (COMPKEY int)
  DECLARE @DRIVERS_SMALL_TRACTIVE_SANITARY_COMPKEYS_FOR_PROCESSING TABLE (COMPKEY int)
    ...
    WHERE 
      (
        (
          [dbo].[ASSETS].diamWidth_inches < @LargeDiameterIn 
          OR 
          [dbo].[ASSETS].diamWidth_inches IS NULL
        ) 
	    AND 
		(
		  [dbo].[ASSETS].height_inches < @LargeDiameterIn 
		  OR 
		  [dbo].[ASSETS].height_inches IS NULL
		)
      )
      AND
      asset_set_id = @asset_set_id
  ...
  INSERT INTO @DRIVERS_LARGE_ROOT_XOR_LARGE_PIPE_COMPKEYS_FOR_PROCESSING
    ...
    (
      (
	    (
	      [ASSETS].diamWidth_inches >= @LargeRootDiameterIn 
          OR 
          [ASSETS].height_inches >= @LargeRootDiameterIn
        )
        AND
        (
          (
            [ASSETS].diamWidth_inches < @LargeDiameterIn 
            OR 
            [ASSETS].diamWidth_inches IS NULL
          )
          AND
          (
            [ASSETS].height_inches < @LargeDiameterIn 
            OR 
            [ASSETS].height_inches IS NULL
          )
        )
      )
      OR
  ...
  WHERE 
    (
      [ASSETS].diamWidth_inches >= @LargeTractiveDiameterIn 
      OR 
      [ASSETS].height_inches >= @LargeTractiveDiameterIn
    )
    AND 
    asset_set_id = @asset_set_id
  ...
  SELECT  A.compkey
  FROM    @DRIVERS_SMALL_TRACTIVE_COMPKEYS_FOR_PROCESSING A 
          INNER JOIN 
          ASSETS B 
          ON 
          (
